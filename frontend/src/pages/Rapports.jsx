...
const paiementIcons = {
  especes: { icon: Banknote, color: "text-emerald-600", bg: "bg-[#D4A853]/10", hex: "#10B981" },
  orange_money: { icon: Smartphone, color: "text-amber-600", bg: "bg-[#D4A853]/10", hex: "#F59E0B" },
  mtn_momo: { icon: Smartphone, color: "text-yellow-500", bg: "bg-[#D4A853]/10", hex: "#EAB308" },
  mobile_money: { icon: Smartphone, color: "text-amber-600", bg: "bg-[#D4A853]/10", hex: "#F59E0B" },
  carte: { icon: CreditCard, color: "text-blue-600", bg: "bg-[#D4A853]/10", hex: "#3B82F6" },
  transfert: { icon: Repeat, color: "text-purple-600", bg: "bg-[#D4A853]/10", hex: "#8B5CF6" },
};
  ...
  return (
    <motion.div ref={reportRef} className="p-6 lg:p-8" variants={containerVariants} initial="hidden" animate="show">
      ...
      <motion.div variants={itemVariants} className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h1 className="text-2xl font-bold text-lounge-100">Rapports</h1>
          </div>
      ...
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        <div className="flex bg-[#1A1714] border border-[#D4A853]/10 rounded-xl p-1">
          {periodTabs.map((pt) => (
            <button key={pt.id} onClick={() => setPeriod(pt.id)}
              className={`flex items-center gap-1.5 h-9 px-3 rounded-lg text-sm font-medium transition-all ${
                period === pt.id ? "bg-[#D4A853] text-lounge-950 shadow-sm" : "text-gray-500 hover:text-gray-700"
              }`}>
              <pt.icon size={14} />
              {pt.label}
            </button>
        ...
        {/* Date nav */}
        <div className="flex items-center gap-2 bg-[#1A1714] border border-[#D4A853]/10 rounded-xl px-2 py-1">
          <button onClick={() => navigateDate(-1)} className="text-lounge-400 hover:text-lounge-200"><ChevronLeft size={16} /></button>
          <input type="date" value={selectedDate} onChange={(e) => setSelectedDate(e.target.value)}
            className="text-sm border-none outline-none bg-transparent text-lounge-200 font-medium" />
          <button onClick={() => navigateDate(1)} className="text-lounge-400 hover:text-lounge-200"><ChevronRight size={16} /></button>
          <button onClick={() => setSelectedDate(new Date().toISOString().split("T")[0])}
            className="text-xs bg-[#2E2822] text-lounge-300 px-2 py-0.5 rounded-lg hover:bg-[#2E2822] font-medium">
            Aujourd'hui
          </button>
        </div>
      </div>
      ...
      {loading && (
        <div className="flex items-center justify-center py-20">
          <div className="animate-spin h-8 w-8 border-2 border-[#D4A853] border-t-transparent rounded-full" />
        </div>
      )}
        ...
        <>
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4 mb-8">
            <motion.div variants={itemVariants}>
              <div className="bg-[#1A1714] rounded-2xl border border-[#D4A853]/10 p-5 shadow-sm">
                ...
                <p className="text-2xl font-bold text-lounge-100">{formatMontant(rapport.ca)}</p>
                {caTrend !== null && (
            ...
            <motion.div variants={itemVariants}>
              <div className="bg-[#1A1714] rounded-2xl border border-[#D4A853]/10 p-5 shadow-sm">
                ...
                <p className="text-2xl font-bold text-lounge-100">{formatMontant(rapport.depenses)}</p>
              </div>
            </motion.div>
            ...
            <motion.div variants={itemVariants}>
              <div className="bg-[#1A1714] rounded-2xl border border-[#D4A853]/10 p-5 shadow-sm">
                ...
                <p className="text-2xl font-bold text-lounge-100">{rapport.nombreCommandes}</p>
          ...
          <div className="grid gap-4 xl:grid-cols-2 mb-8">
            ...
            <motion.div variants={itemVariants} className="rounded-2xl border border-[#D4A853]/10 bg-[#1A1714] p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-lounge-100 mb-1">Paiements par Mode</h3>
                      ...
                      <div key={pm.mode_paiement} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between mb-1.5">
                            <span className="text-sm font-medium text-lounge-200">{paiementLabels[pm.mode_paiement] || pm.mode_paiement}</span>
                            <div className="text-right">
                              <span className="text-sm font-semibold text-lounge-100">{formatMontant(parseFloat(pm.total))}</span>
            ...
            <motion.div variants={itemVariants} className="rounded-2xl border border-[#D4A853]/10 bg-[#1A1714] p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-lounge-100 mb-1">Ventes par Categorie</h3>
              ...
              {rapport.ventesParCategorie && rapport.ventesParCategorie.length > 0 ? (
                <div className="space-y-4">
                  {rapport.ventesParCategorie.slice(0, 8).map((cat, i) => {
                    ...
                    const colors = ["#D4A853", "#10B981", "#EF4444", "#EC4899", "#3B82F6", "#8B5CF6", "#EF4444", "#14B8A6"];
                    return (
                      <div key={i} className="flex items-center gap-3">
                        <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-[#231F1B] shrink-0">
                          <GlassWater size={16} className="text-[#D4A853]" />
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex justify-between mb-1.5">
                            <span className="text-sm font-medium text-lounge-200">{cat.categorie}</span>
                            <div className="text-right">
                              <span className="text-sm font-semibold text-lounge-100">{formatMontant(parseFloat(cat.total))}</span>
                              <span className="ml-1 text-xs text-gray-400">({pct}%)</span>
                            </div>
                          </div>
                          <div className="h-2 bg-lounge-700 rounded-full overflow-hidden">
          ...
          {rapport.commandesParHeure && rapport.commandesParHeure.length > 0 && (
            <motion.div variants={itemVariants} className="rounded-2xl border border-[#D4A853]/10 bg-[#1A1714] p-6 shadow-sm mb-8">
              <h3 className="text-sm font-semibold text-lounge-100 mb-1">Activite par Heure</h3>
              ...
              <ResponsiveContainer width="100%" height={260}>
                <RBarChart data={rapport.commandesParHeure}>
                  <CartesianGrid strokeDasharray="3 4" stroke="#2E2822" vertical={false} />
                  <XAxis dataKey="heure" tick={{ fontSize: 11, fill: "#6B5D50" }} axisLine={false} tickLine={false} />
                  <YAxis tick={{ fontSize: 11, fill: "#6B5D50" }} axisLine={false} tickLine={false} />
                  <Tooltip content={<CustomTooltip />} />
                  <Bar dataKey="commandes" name="Commandes" radius={[6, 6, 0, 0]} fill="#D4A853" fillOpacity={0.8} />
                </RBarChart>
              </ResponsiveContainer>
            </motion.div>
          )}
          ...
          {rapport.topProduits && rapport.topProduits.length > 0 && (
            <motion.div variants={itemVariants} className="rounded-2xl border border-[#D4A853]/10 bg-[#1A1714] p-6 shadow-sm mb-8">
              <div className="flex items-center justify-between mb-4">
                <div>
                  <h3 className="text-sm font-semibold text-lounge-100">Top Produits</h3>
                  <p className="text-xs text-gray-400">Produits les plus vendus</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {rapport.topProduits.slice(0, 6).map((prod, i) => (
                  <div key={i} className="flex items-center gap-3 bg-[#231F1B] rounded-xl p-3">
                    <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#D4A853]/10 text-[#D4A853] text-xs font-bold shrink-0">
                      {i + 1}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-lounge-100 truncate">{prod.nom}</p>
                      <p className="text-xs text-gray-400">{prod.categorie} | {prod.quantite_totale} vendus</p>
                    </div>
                    <p className="text-sm font-semibold text-lounge-100 shrink-0">{formatMontant(parseFloat(prod.CA))}</p>
                  </div>
                ))}
              </div>
            </motion.div>
          ...
          <motion.div variants={itemVariants} className="rounded-2xl border border-[#D4A853]/10 bg-[#1A1714] shadow-sm">
            <div className="flex items-center justify-between border-b border-[#D4A853]/8 px-6 py-4">
              <div>
                <h3 className="text-sm font-semibold text-lounge-100">Liste des Commandes</h3>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-[#D4A853]/8">
                  {(rapport.listeCommandes || []).map((cmd) => (
                    <tr key={cmd.id} className="hover:bg-[#D4A853]/5 transition-colors">
                      <td className="px-6 py-3.5 font-mono text-xs text-gray-500">#{cmd.id}</td>
                      <td className="px-6 py-3.5 font-medium text-lounge-100">{cmd.table_numero || "-"}</td>
                      ...
                      <td className="px-6 py-3.5 text-right font-semibold text-lounge-100">{formatMontant(cmd.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {!loading && !rapport && (
        <div className="flex flex-col items-center justify-center py-20 text-lounge-400">
