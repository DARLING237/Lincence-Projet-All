...
export function ConnectionAlert({ connection }) {
  ...
  return (
    <motion.div
      ...
      className="mb-3 rounded-xl border border-red-500/20 bg-red-500/5 p-4"
    >
      <div className="flex items-start gap-3">
        <AlertTriangle className="mt-0.5 h-5 w-5 text-red-400 flex-shrink-0" />
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <h4 className="text-sm font-semibold text-red-400">Session anormalement longue</h4>
            <Badge variant="error">Alerte</Badge>
          </div>
          <p className="text-xs text-lounge-300 mb-2">
            La session de {connection.utilisateur_nom} a duré {dureeEnMinutes} minutes
          </p>
          <div className="grid grid-cols-2 gap-4 text-xs">
            <div className="flex items-center gap-2 text-lounge-400">
              <Clock size={14} />
